...
public sealed class ToolAuthorizationPolicyTests
{
    ...
    {
        var decision = _policy.Authorize(Request(ToolRiskLevel.Medium), [Permission(ToolRiskLevel.High)]);
    ...
    {
        var request = Request(ToolRiskLevel.Low) with { TenantId = missing == "tenant" ? Guid.Empty : _tenantId, CompanyId = missing == "company" ? Guid.Empty : _companyId, UserId = missing == "user" ? Guid.Empty : _userId, TaskId = missing == "task" ? Guid.Empty : _taskId };
    ...
    {
        var decision = _policy.Authorize(Request(ToolRiskLevel.Low), [Permission(ToolRiskLevel.Critical) with { CompanyId = Guid.NewGuid() }]);
        Assert.False(decision.Allowed);
    ...
    {
        var decision = _policy.Authorize(Request(ToolRiskLevel.Low) with { Resource = "Ledger", Action = "POST" }, [Permission(ToolRiskLevel.High)]);
        Assert.False(decision.Allowed);
        Assert.Equal("no_permission", decision.Reason);
    }

    private ToolAuthorizationRequest Request(ToolRiskLevel risk) => new(_tenantId, _companyId, _userId, _taskId, "ledger", "post", risk);
    private ToolPermission Permission(ToolRiskLevel maximumRisk) => new(_tenantId, _companyId, _userId, "ledger", "post", maximumRisk);
}
