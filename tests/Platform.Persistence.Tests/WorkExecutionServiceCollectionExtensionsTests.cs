...
public sealed class WorkExecutionServiceCollectionExtensionsTests
{
    ...
    public void AddDurableRabbitMqWorkExecution_RegistersScopedPersistenceBoundaryAndHostedConsumer()
    {
        ...
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IWorkStepExecutor)
            && descriptor.ImplementationType == typeof(TestExecutor)
            && descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IWorkDeliveryHandler)
            && descriptor.ImplementationType == typeof(PersistentWorkDeliveryHandler)
            && descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IWorkPublisher)
            && descriptor.ImplementationType == typeof(RabbitMqWorkPublisher)
            && descriptor.Lifetime == ServiceLifetime.Singleton);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IHostedService)
            && descriptor.ImplementationType == typeof(RabbitMqWorkConsumer));
    }
