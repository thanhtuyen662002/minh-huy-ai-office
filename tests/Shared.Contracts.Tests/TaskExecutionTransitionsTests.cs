...
public sealed class TaskExecutionTransitionsTests
{
    ...
    public void CanTransition_RejectsLeavingTerminalStates(TaskExecutionStatus current, TaskExecutionStatus next) => Assert.False(TaskExecutionTransitions.CanTransition(current, next));
}
