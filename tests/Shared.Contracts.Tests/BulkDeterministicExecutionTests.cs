...
public sealed class BulkDeterministicExecutionTests
{
    ...
    [Fact]
    public void ItemIdentity_IsAuthorityAndOperationFenced()
    {
        var plan = Plan();
    ...
    public void ResultRejectsCrossCompanyAndIdentityMismatch()
    {
        ...
        Assert.Throws<UnauthorizedAccessException>(() =>
            BulkDeterministicExecutionContract.ValidateResult(plan, item, result with { CompanyId = Guid.NewGuid() }));
        Assert.Throws<InvalidOperationException>(() =>
            BulkDeterministicExecutionContract.ValidateResult(plan, item, result with { ItemExecutionIdentity = "wrong" }));
    }

    [Fact]
    public void RetryDoesNotReplaySuccessfulWrites()
    {
        var plan = Plan();
    ...
    [Fact]
    public void ResultRequiresExactEvidenceAndAuditCorrelation()
    {
        var plan = Plan();
        var item = plan.Items[0];
        ...
        Assert.Throws<ArgumentException>(() =>
            BulkDeterministicExecutionContract.ValidateResult(plan, item, result with { EvidenceReference = "" }));
        Assert.Throws<InvalidOperationException>(() =>
            BulkDeterministicExecutionContract.ValidateResult(plan, item, result with { AuditCorrelationId = "audit-other" }));
    }
    ...
    private static BulkExecutionPlan Plan()
    {
        ...
        {
            new BulkExecutionItem("ITEM-001", "order.post", "payload-a", "evidence-preview-a"),
            new BulkExecutionItem("ITEM-002", "order.post", "payload-b", "evidence-preview-b")
        }, "audit-123");
    }

    private static BulkExecutionItemResult Result(BulkExecutionPlan plan, BulkExecutionItem item, BulkExecutionItemState state)
        => new(
            plan.Authority.TenantId,
            plan.Authority.CompanyId,
            plan.Authority.DataSourceId,
            item.GetExecutionIdentity(plan.Authority),
            state,
            $"evidence-{item.ItemIdentity}",
            $"actual-{item.ItemIdentity}",
            plan.AuditCorrelationId);
}
