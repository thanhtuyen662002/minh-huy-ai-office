...
const normalizeCompanyOption = (company: CompanyOption): CompanyOption | null => {
  ...
  if (typeof companyId !== "string" || typeof companyName !== "string" || companyId.length === 0 || companyId.trim() !== companyId || companyName.length === 0 || companyName.trim() !== companyName) return null;
...
export function AuthenticatedSessionShell({ state, companies = [], onSelectCompany }: AuthenticatedSessionShellProps) {
  ...
  const readyState = state as Extract<AuthenticatedSessionState, { status: "ready" }>;
  const authoritativeCompanyId = readReadyCompanyId(readyState);
  if (!authoritativeCompanyId) return <InvalidScope />;
  const membership = getOwnDataProperty(readyState, "membership");
  if (membership === null || typeof membership !== "object") return <InvalidScope />;
