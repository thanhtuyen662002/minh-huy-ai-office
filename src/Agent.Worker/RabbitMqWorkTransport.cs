...
{
    /// <summary>
    /// Returns only after the delivery outcome and any checkpoint/completion state are durably committed.
    /// </summary>
    Task<WorkDeliveryResult> HandleAsync(
...
public sealed class RabbitMqWorkConsumer(
    IOptions<RabbitMqWorkOptions> options,
    IWorkDeliveryHandler handler,
    ILogger<RabbitMqWorkConsumer> logger) : BackgroundService
{
    private readonly RabbitMqWorkOptions _options = options.Value;
